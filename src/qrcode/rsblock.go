...
// rsBlockTable holds the blocks of error correction codewords of each
// version and level, in the order L, M, Q, H, as groups of (count, total
...
func (rsblock *qrRSBlock) getRSBlocks(typeNumber int, errorCorrectionLevel errorcorrectionlevel.ErrorCorrectionLevel) []*qrRSBlock {
	rsBlock := rsBlockTable[(typeNumber-1)*4+getTableIndex(errorCorrectionLevel)]
...
// getTableIndex returns the place of a level in the table, which is in the
// order L, M, Q, H; the value of a level is its format information bits.
func getTableIndex(errorCorrectionLevel errorcorrectionlevel.ErrorCorrectionLevel) int {
