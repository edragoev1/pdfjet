...
// images of the grid: the file, what it is, the description of it, and
// whether it carries transparency, which is drawn over a color so it can be
// seen.
var images = [][4]string{
...
// Example17 draws a PNG image of each kind PDFjet reads and says what each one
// is: the color types and the bit depths, the two ways a PNG carries
// transparency, and a PNG that asks to be drawn at 300 dots per inch. The
// images of the grid are from PngSuite, the test images of the PNG format,
// drawn at four times their 32 by 32 pixels so their samples can be seen; the
// samples themselves are checked in pngimage_test.go, against the whole of it.
func Example17() {
