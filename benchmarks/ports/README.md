...
- The files are the same to the byte as at 8db803f0
  ...
  when one font covers it, which is every string here, and it shows most in C#,
  60 to 48 ms; Go's 46 to 43 ms is also its page buffer, which now doubles as it
  fills rather than growing by a quarter. Java's 54 to 56 and Swift's 89 to 90
  are within the noise of these runs. At 7dbfee5d
  (`results/2026-09-16-7dbfee5d.log`) the 500-page times were Java 56, C# 62,
  Go 46 and Swift 90 ms, with the same files.
...
- Go starts a first document fastest, 46 ms against 171 for Java, and keeps the
  smallest process, 11 MB against 79. The JVM and the .NET runtime carry their
...
- The C# figures of the first run at d2f5d4cb were of an old `PDFjet.dll`:
  ...
  names its one source file, and the second run at d2f5d4cb is the one quoted. The run at
