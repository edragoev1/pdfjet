...
| `results/` | The log of the run that the results below come from |
...
AMD Ryzen 5 5600G, 12 threads, Linux, OpenJDK 21.0.12.1, PDFjet at d963a7c8, in one run on a freshly rebooted machine
with nothing else running. Its log is `results/2026-09-15.log`.

### The text document, 500 pages

| Configuration | Time | File | Allocated | Peak memory |
|---|---:|---:|---:|---:|
| PDFjet, IBM Plex Sans | 71 ms | 571,768 bytes | 36 MB | 90 MB |
| PDFjet, Noto Sans | 74 ms | 848,576 bytes | 39 MB | 94 MB |
| iText, IBM Plex Sans subset | 74 ms | 501,395 bytes | 74 MB | 131 MB |
| iText, IBM Plex Sans full | 75 ms | 537,005 bytes | 79 MB | 139 MB |
| iText, Noto Sans subset | 74 ms | 496,442 bytes | 71 MB | 140 MB |
| iText, Noto Sans full | 99 ms | 807,159 bytes | 81 MB | 139 MB |
| PDFBox, Noto Sans subset | 21,339 ms | 426,467 bytes | 38,309 MB | 383 MB |
| PDFBox, Noto Sans full | 19,811 ms | 728,346 bytes | 38,251 MB | 384 MB |

With each page flushed when the next one starts, iText peaked at 126 MB with
IBM Plex Sans and 131 MB with Noto Sans.

| Time by length | 50 pages | 100 pages | 200 pages | 500 pages |
|---|---:|---:|---:|---:|
| PDFjet, IBM Plex Sans | 11 ms | 17 ms | 30 ms | 71 ms |
| iText, IBM Plex Sans subset | 13 ms | 27 ms | 49 ms | 74 ms |
| PDFBox, Noto Sans subset | 2,130 ms | 4,258 ms | 8,354 ms | 21,339 ms |

First document in a new JVM, 20 pages: PDFjet 89 ms, iText 212 ms, PDFBox 1,295 ms.
...
|---|---:|---:|---:|---:|---:|---:|
| PDFjet `BigTable` | 1,930 ms | 2,389 ms | 1,282 MB | 855 MB | 256 MB | 12.3 MB |
| PDFjet `Page` | 2,128 ms | 2,556 ms | 1,460 MB | 775 MB | 256 MB | 12.7 MB |
| iText `PdfCanvas` | 2,029 ms | 2,639 ms | 1,109 MB | 814 MB | 256 MB | 11.8 MB |
| iText `Table` | 33,631 ms | 36,464 ms | 47,121 MB | 6,940 MB | 8 GB | 21.6 MB |
| PDFBox, content stream | 12,537 ms | 13,008 ms | 25,483 MB | 473 MB | 128 MB | 11.9 MB |
| PDFjet `Page`, page by page | 2,090 ms | 2,440 ms | 1,460 MB | 392 MB | 32 MB | 12.7 MB |
| iText `PdfCanvas`, page by page | 2,029 ms | 2,529 ms | 1,133 MB | 406 MB | 32 MB | 11.8 MB |
...
Two earlier runs the same day measured `BigTable` at 1,945 and 2,011 ms and
`PdfCanvas` at 2,030 and 2,025 ms.

### What the numbers say

- PDFjet and iText's low-level API take about the same time for long documents
  (71 and 74 ms for 500 pages); PDFjet is faster on shorter ones and allocates half as much, with a lower peak, a
  ...
  applies to every line, and 6.5 times slower on the table.
- On the table, `BigTable` is 5% faster than iText's `PdfCanvas` (1,930 against
  2,029 ms), and iText's own `Table` is 17 times slower and needs an 8 GB heap.
- Drawn with the same calls, PDFjet is behind iText: on `Page` the table takes
  5% longer than on `PdfCanvas` and allocates 32% more; page by page, 3% longer
  ...
  position, which makes `BigTable` take 9% less time than the same drawing on
  `Page`.
