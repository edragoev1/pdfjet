...
public class TextBlock : IDrawable {
        ...
        foreach (String line in lines) {
            ...
            if (this.font.StringWidth(fallbackFont, fontSize, text) <= textAreaWidth) {
                ...
                if (Util.IsCJK(text)) {
                    ...
                    String lastLine = Util.Trim(sb.ToString());
                    if (lastLine.Length > 0) {
                        textLines.Add(new TextLine(font, lastLine));
                    }
                } else {
                    StringBuilder sb = new StringBuilder();
    ...
    private TextLine NewTextLine(String text, int from) {
        int to = text.Length;
        while (to > from && Util.IsJavaWhitespace(text[to - 1])) {
            to--;
        }
    ...
    private static int NextCharacterBreak(String word, int i) {
        int ch = Util.CodePointAt(word, i);
        i += (ch > 0xFFFF) ? 2 : 1;
        while (IsLeadingVowel(ch) && i < word.Length) {
            ch = Util.CodePointAt(word, i);
            i += (ch > 0xFFFF) ? 2 : 1;
        }
        while (i < word.Length && StaysWithPrevious(Util.CodePointAt(word, i))) {
            i += (Util.CodePointAt(word, i) > 0xFFFF) ? 2 : 1;
        }
        return i;
    }
    ...
    private void AddRightToLeftLines(List<TextLine> textLines, string paragraph, float textAreaWidth) {
        ...
        StringBuilder sb = new StringBuilder();
        int from = 0;
        foreach (string token in Util.SplitOnWhitespace(paragraph)) {
