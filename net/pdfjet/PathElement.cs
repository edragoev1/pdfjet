...
class PathElement {
    ...
    public PathElement(Char command) {
        this.command = command;
        this.arguments = new List<String>();
    }

    public List<PathElement> getOperations() {
        List<PathElement> operations = new List<PathElement>();
        int n = getNumberOfArguments();
        PathElement operation = new PathElement(command);
        foreach (String argument in arguments) {
            operation.arguments.Add(argument);
            if (operation.arguments.Count % n == 0) {
                operations.Add(operation);
                operation = new PathElement(command);
            }
        }
