...
public class Bidi {
    ...
    public static String reorderVisually(String str, int from, int to) {
        ...
        while (i >= 0) {
            ...
            if (isArabicLetter(ch)) {
                ...
                // draw it, but puts it in the actual text of the glyph after
     ...
     */
    static String lettersOf(int ch) {
        if (ch >= 0xFEF5 && ch <= 0xFEFC) {             // the lam-alef ligatures
            final int[] alefs = {0x0622, 0x0623, 0x0625, 0x0627};
            return new String(new int[] {alefs[(ch - 0xFEF5) / 2], 0x0644}, 0, 2);
        }
        if (ch < 0xFB50) {
            return null;
        }
        for (int i = 0; i < forms.length; i += 5) {
            for (int j = i + 1; j < i + 5; j++) {
                if (forms[j] == ch) {
                    return new String(Character.toChars(forms[i]));
                }
            }
        }
        return null;
     ...
     */
    static Integer mirrored(int ch) {
        switch (ch) {
        case '(':  return (int) ')';
        case ')':  return (int) '(';
        case '[':  return (int) ']';
