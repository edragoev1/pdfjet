...
@Suite struct GS1Tests {
    @Test func digitalLinkPutsTheKeyAndItsQualifiersInThePath() throws {
        let cases = [
        ...
        for (domain, data, want) in cases {
            #expect(try GS1.digitalLink(domain: domain, data: data) == want)
        }
    }

    @Test func digitalLinkRefusesWhatIsNotOne() {
        let domain = "The domain of a GS1 Digital Link starts with https:// or http://, such as https://id.gs1.org!"
        let cases = [
        ...
        for (d, data, message) in cases {
            let error = #expect(throws: PDFjetError.self) { _ = try GS1.digitalLink(domain: d, data: data) }
